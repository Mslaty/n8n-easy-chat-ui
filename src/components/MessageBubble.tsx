  ...
  return (
    <div className={`flex mb-4 ${isUser ? 'justify-end' : 'justify-start'} message-appear group relative`}
      ...
      {isUser && showActions && !message.isTyping && !isOnlyVoiceMessage() && (
        <div className="absolute left-0 top-1/2 transform -translate-x-2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity">
          <button 
            onClick={handleCopy} 
      ...
      {!isUser && showActions && !message.isTyping && !isOnlyVoiceMessage() && (
        <div className="absolute right-0 top-1/2 transform translate-x-2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity">
          <button 
            onClick={handleCopy} 
            className="text-xs text-gray-400 p-1 hover:text-white rounded bg-chat-dark-secondary" 
            aria-label="Copy message"
