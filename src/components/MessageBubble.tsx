...
const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  return <div className={`flex mb-4 ${isUser ? 'justify-end' : 'justify-start'} message-appear`}>
      <div className={`rounded-lg px-4 py-2 max-w-[80%] break-words ${isUser ? 'bg-chat-user-bubble text-white' : 'bg-chat-agent-bubble text-gray-200'} ${message.isTyping ? 'animate-pulse' : ''}`} onMouseEnter={() => setShowActions(true)} onMouseLeave={() => setShowActions(false)}>
        {message.content && <div className="mb-2">
            {message.isTyping ? <span className="typing-indicator">{message.content}</span> : <div className="whitespace-pre-wrap">{message.content}</div>}
          </div>}
            ...
            {message.attachments.map(attachment => <div key={attachment.id || attachment.name} className="">
                {attachment.previewUrl && isImageFile(attachment.data) ? <div className="relative group">
                    ...
                    <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-30 transition-opacity flex items-center justify-center opacity-0 group-hover:opacity-100">
                      <button onClick={() => onDownloadAttachment(attachment)} className="p-2 rounded-full bg-gray-800 bg-opacity-70 text-white hover:bg-opacity-100">
                        <Download size={18} />
                      </button>
                    </div>
                  </div> : attachment.data && isAudioFile(attachment.data) ? <div className="flex items-center p-2 bg-gray-800 bg-opacity-30">
                    <button onClick={() => toggleAudioPlayback(attachment)} className="p-2 rounded-full bg-gray-700 mr-3 text-white hover:bg-gray-600">
        ...
        {showActions && !message.isTyping && <div className="flex justify-end mt-1">
            <button onClick={handleCopy} className="text-xs text-gray-400 p-1 hover:text-white" aria-label="Copy message">
              <Copy size={14} />
            </button>
          </div>}
